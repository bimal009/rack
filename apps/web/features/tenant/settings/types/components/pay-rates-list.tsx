...
import { generateTypeId, initialPayRatePolicies } from "../lib/data"
import type { PayRatePolicy, PayRatePolicyInput } from "../lib/schema"
...
function earningsLabel(policy: PayRatePolicy) {
  const parts: string[] = []
  if (policy.perSessionRate) {
    parts.push(`${currency.format(policy.perSessionRate)}/session`)
  }
  if (policy.revenueSharePercent) {
    parts.push(`${policy.revenueSharePercent}% revenue`)
  }
...
export function PayRatesList() {
  ...
  const columns = useMemo(() => {
    ...
    return columnHelper.columns([
      ...
      columnHelper.accessor("entranceMethod", {
        header: "Entrance Method",
        enableGlobalFilter: false,
        cell: ({ getValue }) => (
