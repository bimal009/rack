...
import { FEATURE_OPTIONS } from "@repo/types"
...
export function FeaturesStep({ value, error, onChange }: FeaturesStepProps) {
  const [draft, setDraft] = useState("")

  function add(feature: string) {
    const name = feature.trim()
    if (!name) return
    if (value.some((f) => f.toLowerCase() === name.toLowerCase())) return
    onChange([...value, name])
    setDraft("")
  }
  ...
  const suggestions = FEATURE_OPTIONS.filter(
    (option) => !value.some((f) => f.toLowerCase() === option.toLowerCase())
  )
    ...
    <div className="space-y-6">
      <div className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          Type a feature and press Enter. Add your own or pick from common ones.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <Input
          ...
          onKeyDown={onKeyDown}
          placeholder="e.g. Free Weights, Sauna, Parking"
